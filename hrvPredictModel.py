...
def new_model(input_length):
    ...
    rmssd_branch = tf.keras.layers.Conv1D(filters=128, kernel_size=7, activation='relu', padding='same')(x)
    ...
    # Model
    model = tf.keras.Model(inputs=input_signal, outputs={'sdnn': sdnn_output, 'rmssd': rmssd_output})
    return model
...
if __name__ == "__main__":
   ...
   # model = new_model(input_length, len(METRICS))
    ...
    # Przygotowanie wyjść dla modelu wielowymiarowego
    y_train = {'sdnn': y_train[:, 0], 'rmssd': y_train[:, 1]}
    y_val = {'sdnn': y_val[:, 0], 'rmssd': y_val[:, 1]}
