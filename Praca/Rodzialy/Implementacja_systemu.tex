...
próbkowania (2 Hz). Warunek ten okreœla maksymaln¹ czêstotliwoœæ sygna³u, któr¹ mo¿na poprawnie odwzorowaæ bez wyst¹pienia aliasingu – zjawiska nak³adania siê ró¿nych sk³adowych czêstotliwoœciowych. W efekcie aliasing prowadzi³by do zniekszta³ceñ sygna³u, uniemo¿liwiaj¹c jego prawid³ow¹ rekonstrukcjê. Spe³nienie kryterium Nyquista gwarantuje, ¿e sygna³ jest próbkowany z odpowiedni¹ czêstotliwoœci¹, co umo¿liwia zachowanie jego integralnoœci i poprawnoœæ analizy \cite{Wang-Nyquist-Sampling} .


\subsection{Przygotowanie sygna³u do analizy}
Przed wykonaniem transformacji na sygna³ na³o¿ono \textit{okno Hanninga}, które wyg³adza dane na pocz¹tku i koñcu sygna³u, redukuj¹c amplitudy w tych obszarach. Ka¿dy punkt sygna³u $x(n)$ zosta³ pomno¿ony przez odpowiedni¹ wartoœæ funkcji okna $w(n)$, zdefiniowan¹ w nastêpuj¹cy sposób:

\begin{equation}
    w(n) = 0.5 \left( 1 - \cos\left(\frac{2\pi n}{N-1}\right) \right),
\end{equation}

gdzie $n$ to indeks próbki, a $N$ to liczba próbek sygna³u. Zastosowanie tego okna pozwala na wyg³adzenie danych poprzez stopniowe t³umienie wartoœci na krañcach sygna³u oraz minimalizacjê efektów nieci¹g³oœci, takich jak nag³e skoki wartoœci na pocz¹tku i koñcu, które mog³yby wprowadzaæ zak³ócenia w analizie czêstotliwoœciowej \cite{Pielawski-Hann-Windows}. Nastêpnie wykonano dyskretn¹
...
\section{Repozytorium z wytworzonym kodem}
Repozytorium zawieraj¹ce kody Ÿród³owe aplikacji stworzonych na potrzeby niniejszej pracy dyplomowej znajduj¹ siê pod adresem:\href{https://github.com/PiotrexOG/ECG}{https://github.com/PiotrexOG/ECG}.

\noindent Poni¿ej przedstawiono krótki opis zawartoœci plików, u³atwiaj¹cy nawigacjê po repozytorium:
\begin{itemize}
    \item config.py -- parametry konfiguracji systemu, takie jak tryb pracy aplikacji, czêstotliwoœæ próbkowania, adres serwera
    \item main.py -- g³ówny plik aplikacji, zawiera g³ówny punkt wejœcia oraz implementacje serwera.
    \item dataSenderEmulator.py -- funkcje odpowiedzialne za wczytanie danych z pliku oraz symulacje.
    \item doba\_analysis.py -- funkcje odpowiedzialne za analize danych HRV dla danych ca³odobowych
    \item EcgData.py -- klasa odpowiedzialna za przechowywanie i analize danych EKG
    \item RPeaksFinder.py -- klasa abstrakcyjna, definiuj¹ca metody do znajdowania szczytów R w sygnale EKG
    \item UNetFinder.py -- implementacja RPeaksFinder korzystaj¹ca z sieci UNet
    \item PanTompkinsFinder.py -- implementacja RPeaksFinder korzystaj¹ca z algorytmu Pan-Tompkins
    \item EcgPlotter.py -- klasa rysuj¹ca wykres EKG.
    \item FFTPlotter.py -- klasa rysuj¹ca analizê spektraln¹ przy u¿yciu FFT
    \item HRPlotter.py -- klasa rysuj¹ca wykresy têtna, interwa³ów RR oraz ich przefiltrowanych wersji
    \item models.py -- implementacja modelu UNet
    \item hrvPredictionModels.py -- implementacje modeli neuronowych do przewidywania miar HRV, model end-to-end oraz model oparty o interwa³y RR
    \item merge\_csvs.py -- funkcje odpowiedzialne za ³¹czenie wielu plików CSV w jeden
    \item nnHelpers.py -- funkcje pomocnicze do dzia³ania modelu UNet
    \item PanTompkins.py -- implementacja algorytmu Pan-Tompkins
    \item FFT.py -- funkcje odpowiedzialne za obliczanie mocy pasm LF i HF
    \item trainModel.py -- funkcje odpowiedzialne za trening sieci UNet
\end{itemize}
