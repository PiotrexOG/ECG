...
W niniejszej pracy zdecydowano siê na wykorzystanie czujnika Polar H10 ze wzglêdu na jego udokumentowan¹ dok³adnoœæ w rejestrowaniu sygna³u EKG  \cite{Gilgen-Ammann-RR}. Urz¹dzenie zapewnia czêstotliwoœæ próbkowania na poziomie 130 Hz, co jest wartoœci¹ wystarczaj¹c¹ do analiz sygna³u EKG, takich jak wykrywanie szczytów R. Dodatkow¹ zalet¹ jest prostota u¿ytkowania – wystarczy jeden pas zak³adany na klatkê piersiow¹, co sprawia, ¿e urz¹dzenie jest wygodniejsze w u¿yciu ni¿ profesjonalne systemy, takie jak Holter, wymagaj¹ce wielu odprowadzeñ i bardziej z³o¿onej interpretacji wyników.
Na Rys. \ref{fig/polarh10} przedstawiono wygl¹d czujnika Polar H10.

\begin{figure}[ht]
    \centering
    \includegraphics[scale=0.15]{Rysunki/polarh10.png}
    \caption{Nadajnik na klatkê pierisow¹ Polar H10.}
    \caption*{ród³o:  \url{https://www.polar.com/pl/sensors/h10-heart-rate-sensor} [Data uzyskania dostêpu 10.12.2024]}
    \label{fig/polarh10}
\end{figure}

Polar H10 oferuje równie¿ otwarty kod Ÿród³owy aplikacji do ³¹czenia z urz¹dzeniem mobilnym, co pozwala na ³atwe u¿ycie najwa¿niejszych funkcjonalnoœci we w³asnej aplikacji. Zestaw narzêdzi wykorzystywanych przez programistów do tworzenia aplikacji (\textit{SDK} (ang. \textit{Software Development Kit})) oraz regu³ i protoko³ów umo¿liwiaj¹cych wspó³dzia³anie ró¿nych komponentów (\textit{API} (ang. \textit{Application Programming Interface})) jest bardzo dobrze udokumentowany i udostêpniony  na licencji umo¿liwiaj¹cej wykorzystanie zarówno do celów prywatnych, jak i komercyjnych \cite{Polar-BLE-SDK}.
...
\section{Aplikacje i Protokó³ BLE \small{(autor: Piotr Weso³owski)}}
Obecnie (stan na 10.12.2024) dostêpne s¹ aplikacje umo¿liwiaj¹ce po³¹czenie z czujnikiem Polar H10,
...
danych. Dane te s¹ przechowywane w pamiêci urz¹dzenia. Na Rys. \ref{subfig:1} przedstawiono wygl¹d menu g³ównego zaimplementowanej aplikacji, a na Rys. \ref{subfig:1} efekt dzia³ania funkcjonalnoœci wyœwietlania sygna³u EKG w formie wykresu w czasie rzeczywistym.

\begin{figure}[h!]
    \centering
    \subfloat[Menu g³ówne\label{subfig:1}]
    {\includegraphics[width=.48\linewidth]{Rysunki/polargraphMENU.jpg}}
    \hspace{0.5cm} % Odstêp poziomy miêdzy zdjêciami
    \subfloat[Zak³adka analizy EKG\label{subfig:2}]
    {\includegraphics[width=.48\linewidth]{Rysunki/polargraph_ekg.jpg}}
    \vspace{0.5cm} % Odstêp poziomy miêdzy zdjêciami
...
Funkcjonalnoœci zaimplementowane w kodzie, korzystaj¹ z interfejsu API Polar, aby w czasie rzeczywistym pobieraæ dane z czujnika EKG. Dane s¹ przesy³ane w paczkach, z których ka¿da zawiera oko³o 73 próbki. Liczba ta wynika z implementacji API producenta i jest optymalizowana pod k¹tem efektywnoœci transmisji oraz niskiego opóŸnienia w technologii BLE. Ka¿da próbka sk³ada siê z wartoœci potencja³u elektrycznego w mikrovoltach (\textmu{}V) (ang. \textit{Voltage}) oraz znacznika czasowego (ang. \textit{Timestamp}). Czujnik wysy³a dane sk³adaj¹ce siê z timestampu, który mierzy liczbê nanosekund, jakie up³ynê³y od pewnego momentu w czasie. Domyœlnie, zgodnie z UNIX, tym momentem jest 1970-01-01T00:00:00Z. Jednak w przypadku tego urz¹dzenia punktem odniesienia jest 2019-01-01T00:00:00Z (599616000000000000 nanosekund). Decyzja ta wynika z ograniczeñ pamiêci poprzednich modeli oraz chêci zachowania spójnoœci przez producenta \cite{Polar-BLE-SDK}.
...
W pracy, poza zbiorem danych zebranym przy u¿yciu sensora Polar H10, wykorzystano równie¿ zbiory danych pochodz¹ce z zewnêtrznych Ÿróde³. Pos³u¿y³y one jako dodatkowy materia³ do nauki modeli oraz weryfikacji ich skutecznoœci w analizie sygna³ów EKG. 
