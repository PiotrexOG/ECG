\section{Przewidywanie HRV przy u¿yciu sieci neuronowych i danych RR \small{(autor: Jakub Wojtalewicz)}}\label{RRsiec}
...
\begin{itemize}
    \item \textbf{Œredni b³¹d bezwzglêdny (ang. Mean Absolute Error, MAE)} -- œrednia arytmetyczna wartoœci bezwzglêdnych ró¿nic miêdzy wartoœciami rzeczywistymi a przewidywanymi.
          \begin{equation}
              \text{MAE} = \frac{1}{n} \sum_{i=1}^n |y_i - \hat{y}_i|
          \end{equation}

    \item \textbf{B³¹d œredniokwadratowy (ang. Mean Squared Error, MSE)} -- œrednia arytmetyczna
    ...
    \item \textbf{Pierwiastek ze œredniego b³êdu kwadratowego (ang. Root Mean Squared Error, RMSE)} --
    ...
    \item \textbf{Wspó³czynnik determinacji ($R^2$)} -- miara jakoœci dopasowania modelu do danych.
          Reprezentuje jaka czêœæ zmiennoœæi wartoœci w danych jest wyjaœniana przez
          model. Wartoœæ $R^2$ przyjmuje wartoœci z zakresu od $-\infty$ do 1, gdzie 1
          oznacza idealne dopasowanie, a wartoœci poni¿ej 0 sugeruj¹, ¿e lepsze przewidywanie uzyska³oby siê przy u¿yciu zwyk³ej œredniej.
...
\subsection{Wyniki}
Œrednie wyniki uzyskane w ramach walidacji krzy¿owej dla wybranych modeli przedstawiono w tabelach \ref{wyniki_hrv_pojedyncze_rr}, \ref{wyniki_hrv_wiele_rr} oraz \ref{wyniki_hrv_wiele_rr2}.
\begin{table}[ht]
    \captionsetup{justification=centering}
    \caption{Œrednie wyniki walidacji krzy¿owej dla modeli przewiduj¹cych pojedyncze miary (zaokr¹glone do 2 miejsc dziesiêtnych)}
    \centering
...
\begin{table}[ht]
    \captionsetup{justification=centering}
    \caption{Œrednie wyniki walidacji krzy¿owej dla modeli przewiduj¹cych wiele miar (zaokr¹glone do 2 miejsc dziesiêtnych)}
    \centering
    \begin{tabular}{|l|c|c|c|c|c|c|}
        \hline
...
Spoœród modeli przewiduj¹cych pojedyncze miary najlepsze wyniki uzyskano dla
SDNN ($R^2 = 0,93 \pm 0,02$), co wskazuje na wysokie pokrycie z
rzeczywistoœcia. RMSSD uzyska³o ni¿sze lecz dalej wysokie $R^2 = 0,74 \pm
    0,15$. Ró¿nica miêdzy dopasowaniem tych metryk, mo¿e wynikaæ z wy¿szej
zmiennoœæi RMSSD w krótszych okresach czasowych. Wariant modelu przewiduj¹cy
obie te miary jednoczeœnie wykaza³ podobnymi lub lepszymi wynikami wzglêdem
wariantów z jêdna cech¹ - dla SDNN uzyskano zbli¿ony wynik dopasowania, a dla
RMSSD lepszy ($R^2 = 0,84 \pm 0,12$). Pomimo dobrego ogólnego dopasowania,
zaobserwowano pogorszenie jakoœci przewidywañ w poszczególnych przypadkach, co
potwierdzaj¹ wy¿sze wartoœci MAE i MSE.

Model przewiduj¹cy cztery miary jednoczeœnie osi¹gn¹³ mieszane wyniki. Dla
wartoœci LF i RMSSD uzyska³ wspó³czynnik determinacji na poziomie oko³o 0,75,
natomiast dla pozosta³ych metryk rezultaty by³y mniej zadowalaj¹ce.

\subsection{Wnioski}
Wiele z analizowanych wariantów modeli wykaza³o dobre dopasowanie do
rzeczywistych wartoœci. Jednak, maj¹c na uwadze fakt, ¿e modele otrzymywa³y na
wejœciu interwa³y RR, z których te miary mo¿na obliczyæ w sposób bezpoœredni,
uzyskane wyniki nie spe³ni³y w pe³ni oczekiwañ. Gdyby jednak uda³o siê
zintegrowaæ przedstawiony model z sieci¹ UNet w ramach systemu end-to-end,
uzyskane rezultaty mog³yby zostaæ uznane za satysfakcjonuj¹ce.

\FloatBarrier
\section{Przewidywanie miar HRV przy u¿yciu sieci typu end-to-end \small{(autor: Jakub Wojtalewicz)}}
...
eksperymencie \ref{sczyty-eksperyment}.
...
liczbê miar ni¿ dwie, jednak ¿aden z tych modeli nie osi¹gn¹³ zadowalaj¹cych
wyników.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{Rysunki/RMSSD_2_folds.png}
