...
def create_hrv_model(input_length, output_len):
    ...
    x = Conv1D(filters=64, kernel_size=7, activation="relu", padding="same")(
        input_signal
    )
...
def end_to_end_model(input_length, output_len):
    ...
    x = Conv1D(filters=32, kernel_size=7, activation="relu", padding="same")(
        input_signal
    )
    x = BatchNormalization()(x)
...
def get_patient_data_csv(path, finder, stride):
    ...
    # )
    X_train, y_train = data.extract_signal_wind_hrv(
        input_length, METRICS, stride
    )  # , int(0.8 * input_length))
    return X_train, y_train
...
if __name__ == "__main__":
    ...
    X, y = get_patients_data_csv(
        "data", [csvs[2], csvs[3], csvs[4], csvs[24]], finder
    )  # , int(0.8*input_length))
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        model = end_to_end_model(input_length, len(METRICS))
        model.compile(
        ...
        for i in range(len(METRICS)):
            ...
            create_scatter_line_plots(
                f"{METRICS[i]} - Fold {fold +1}", real, pred, f"hrvPlots\\FOLD{fold+1}"
            )
    ...
    for i in range(len(METRICS)):
        print(f"### {METRICS[i]}")
        print(
            f"Średnie MAE: {np.mean(mae_per_metric_list[i]):.2f} ± {np.std(mae_per_metric_list[i]):.2f}"
        )
        print(
            f"Średnie MSE: {np.mean(mse_per_metric_list[i]):.2f} ± {np.std(mse_per_metric_list[i]):.2f}"
        )
        print(
            f"Średnie RMSE: {np.mean(rmse_per_metric_list[i]):.2f} ± {np.std(rmse_per_metric_list[i]):.2f}"
        )
        print(
            f"Średnie R^2: {np.mean(r2_per_metric_list[i]):.2f} ± {np.std(r2_per_metric_list[i]):.2f}"
        )
